...
# Koneksi DB
def init_db():
    conn = sqlite3.connect("asdp.db")
    c = conn.cursor()
    c.execute("""CREATE TABLE IF NOT EXISTS tiket_terjual (
        ...
        pelabuhan TEXT,
        jumlah INTEGER)""")
    c.execute("""CREATE TABLE IF NOT EXISTS penambahan (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        nama_file TEXT,
        tanggal_upload TIMESTAMP,
        tanggal DATE,
        pelabuhan TEXT,
        nilai INTEGER)""")
    c.execute("""CREATE TABLE IF NOT EXISTS pengurangan (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        nama_file TEXT,
        tanggal_upload TIMESTAMP,
        tanggal DATE,
        pelabuhan TEXT,
        nilai INTEGER)""")
    c.execute("""CREATE TABLE IF NOT EXISTS golongan (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        nama_file_invoice TEXT,
        nama_file_summary TEXT,
        tanggal_upload TIMESTAMP,
        tanggal_mulai DATE,
        tanggal_selesai DATE,
        pelabuhan TEXT,
        nilai INTEGER)""")
    c.execute("""CREATE TABLE IF NOT EXISTS rekonsiliasi (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        nama_file_invoice TEXT,
        nama_file_koran TEXT,
        tanggal_upload TIMESTAMP,
        tanggal DATE,
        harga REAL,
        kredit REAL,
...
if menu == "Dashboard":
    ...
    st.dataframe(df_final, use_container_width=True)elif menu == "Tiket Terjual":
    ...
    if files:
        ...
        st.info("Silakan upload file tiket.")elif menu == "Penambahan & Pengurangan":
    ...
    if f_inv and f_tik:
        try:
            ...
            st.session_state['golongan'] = df_final  # disimpan untuk dashboard
            st.dataframe(df_final, use_container_width=True)
        except Exception as e:
            st.error(f"Gagal memproses file: {e}")
    else:
        st.info("Silakan upload file invoice dan tiket.")elif menu == "Rekonsiliasi":
